...
const Syllabus = ({route}) => {
    ...
    let title = 'Syllabus for ' + coursesCode;


    const get = async ()  =>{
        const getData = [];

        const querySnapshot = await getDocs(collection(db, 'Course Details', coursesCode, "Syllabus"));
        ...
        return(
            <View style={styles.Container}>
                ...
                {data.map((data)=>{
                    ...
                    return(
                        <TouchableOpacity key={data.key} activeOpacity={0.7} style={styles.secondaryContainer} onPress={()=> {navigation.navigate('SyllabusDoc', {
                            uriPdf: uriLink,
                        })}}>
