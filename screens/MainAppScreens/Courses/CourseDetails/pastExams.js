...
const Exams = ({route}) => {
    ...
    let title = 'Past Exams for ' + coursesCode;


    const get = async ()  =>{
        const getData = [];

        const querySnapshot = await getDocs(collection(db, 'Course Details', coursesCode, "Exams"));
...
const navigation = useNavigation();
        ...
        return(
            <View style={styles.Container}>
                ...
                {data.map((data)=>{
                    ...
                    return(
                        <TouchableOpacity key={data.key} activeOpacity={0.7} style={styles.secondaryContainer} onPress={()=> {navigation.navigate('ExamsDoc', {
                            uriPdf: uriLink,
                        })}}>
    ...
    return(
        <SafeAreaView style={{flex: 1, }}>
            <Stack.Navigator>
                <Stack.Screen name={title} component={Exam} options={{headerTitle:title, headerShadowVisible:false,
                    headerTintColor: color.primary,
                    headerStyle: { backgroundColor: color.background },
                    headerTitleStyle: {fontFamily: font.semiBold}}}/>
                <Stack.Screen name='ExamsDoc' component={OnExamsDoc} options={{headerTitle : ''}}/>
            </Stack.Navigator>
        </SafeAreaView>   
    )
}
