...
const HealthSchools = ({navigation}) => {

    const [healthSchoolsData, setHealthSchoolsData] = useState([])
    const getHealthSchools = async ()  =>{
        const getHealthSchoolsData = [];

        const querySnapshot = await getDocs(collection(db, "School", "1", "Health Science"));
        
        querySnapshot.forEach((doc) => {
        console.log(doc.id, " => ", doc.data());
        getHealthSchoolsData.push({
            ...doc.data(),
            key: doc.id,
        });
    //    return () => querySnapshot(); 
    });
    setHealthSchoolsData(getHealthSchoolsData);
    }
